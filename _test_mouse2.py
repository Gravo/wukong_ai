...
for i in range(min(50, len(dataset))):
    frames_np, action_gt, mouse_tgt = dataset[i]  # frames_np: (12,224,224) numpy
    frames_t = torch.from_numpy(frames_np).unsqueeze(0).to(device)  # (1,12,224,224)
    with torch.no_grad():
        ...
        acts.append(pred)
        if i < 5:
            print(f"  sample {i}: pred={names[pred]:6s}, dx={dx:.4f}, dy={dy:.4f}, gt_dx={mouse_tgt[0]:.4f}")
