# -*- coding: utf-8 -*-
"""
Created on Sun Sep 15 19:10 2024
...
if __name__ == '__main__':
    ...
    agent = PPO(WIDTH, HEIGHT, action_size, model_path, log_path)
    # DQN init
    ...
    for episode in range(EPISODES):
        ...
        # t_screen_rgb = cv2.cvtColor(t_screen, cv2.COLOR_BGR2RGB)
        # cv2.imwrite(save_path, t_screen_rgb)  # 将归一化后的图像恢复原样再保存
        ...
        station = cv2.resize(t_screen_rgb,(WIDTH,HEIGHT))
        station = station.transpose(2, 0, 1)  # 将维度从 (height, width, channels) 转换为 (channels, height, width)

        station = torch.FloatTensor(station).unsqueeze(0)  # 添加批次维度
        ...
        while True:
            ...
            action = agent.select_action(station)
            take_action(action)
            # take station then the station change

            t_screen_next = grab_screen(window_size)
            # 将图像转换为RGB
            t_screen_next_rgb = t_screen_next[:, :, :3]
            next_station = cv2.resize(t_screen_next_rgb, (WIDTH, HEIGHT))
            next_station = next_station.transpose(2, 0, 1)  # 将维度从 (height, width, channels) 转换为 (channels, height, width)
            next_station = torch.FloatTensor(next_station).unsqueeze(0)  # 添加批次维度
            ...
            if emergence_break == 100:
                ...
                agent.save_model()
                paused = True
            agent.store_transition(station, action, reward, next_station, done)
            ...
            # 将图像转换为 BGR 格式
            # new_img = cv2.cvtColor(screen_next_rgb_original, cv2.COLOR_RGB2BGR)
            new_img = cv2.cvtColor(t_screen_next, cv2.COLOR_RGB2BGR)
