"""
data_collector_v3.py - 数据采集脚本 v3.0
录制人类玩家的游戏画面+操作，支持 GOAL 标注，用于 Goal-Conditioned Behavior Cloning

新增功能（相比v2.1）：
1. ✅ Goal 标注：录制前定义目标序列，录制中按 'G' 标记「到达当前目标」
2. ✅ 自动 goal_id 标注：每帧都带有当前 goal_id
3. ✅ H5 格式更新：新增 "goal_ids" 数据集
4. ✅ 质量报告更新：显示 goal 分布
...
使用方法：
  ...
  # 1. 创建 goals.txt（每行一个目标）
  echo 存档点A > goals.txt
  echo 岔路口1 >> goals.txt
  echo BOSS门 >> goals.txt
  echo 存档点B >> goals.txt
  
  # 2. 开始录制（指定 --goals-file）
  ...
  # 在游戏中：
  #   - 正常游玩
  #   - 到达一个目标后，按 'G' 标记（脚本自动切换到下一个目标）
  #   - ESC 停止录制
  
  # 3. 查看录制的 goal 分布
  C:\Python\python.exe -u training/data_collector_v3.py --report pathfinding_data

目标序列示例（黑神话悟空-第一章）：
  存档点A → 大树守卫 → 岔路口（左去BOSS/右去道具） → BOSS门 → 存档点B
"""
...
class GoalDataCollector:
    ...
    def load_goals(self, goals_file):
        """从文件加载目标序列（每行一个目标）"""
        if not os.path.exists(goals_file):
            print(f"[Goal] ❌ Goals file not found: {goals_file}", flush=True)
            return False

        with open(goals_file, 'r', encoding='utf-8') as f:
            self._goals = [line.strip() for line in f if line.strip()]

        if len(self._goals) == 0:
            print(f"[Goal] ❌ No goals found in file", flush=True)
            return False

        self._current_goal_id = 0
        self._goal_reached = [False] * len(self._goals)

        print(f"[Goal] ✅ Loaded {len(self._goals)} goals:", flush=True)
    ...
    def set_goals(self, goals_list):
        """直接设置目标序列（列表）"""
        self._goals = list(goals_list)
        self._current_goal_id = 0
        self._goal_reached = [False] * len(self._goals)

        print(f"[Goal] ✅ Set {len(self._goals)} goals:", flush=True)
        for i, g in enumerate(self._goals):
            print(f"  {i}: {g}", flush=True)

    def advance_goal(self):
        """标记当前目标已到达，切换到下一个目标"""
        if self._current_goal_id < len(self._goals):
            self._goal_reached[self._current_goal_id] = True
            print(f"\n[Goal] ✅ Reached: {self._goals[self._current_goal_id]} "
                  f"(goal_id={self._current_goal_id})\n", flush=True)

            self._current_goal_id += 1

            if self._current_goal_id >= len(self._goals):
                print(f"\n[Goal] 🎉 All goals reached! Still recording...\n", flush=True)
            else:
                print(f"\n[Goal] ➡️  Next goal: {self._goals[self._current_goal_id]} "
                      f"(goal_id={self._current_goal_id})\n", flush=True)

    def get_current_goal_id(self):
        """获取当前帧的 goal_id"""
        return min(self._current_goal_id, len(self._goals) - 1) if self._goals else 0
    ...
    def setup_keyboard_listener(self):
        ...
        # 监听的按键
        MONITORED = {'w', 'a', 's', 'd', 'j', 'k', 'v', 'r', 'space', 'shift', 'g'}
    ...
    def _save_h5(self, path, mode, frames, actions, timestamps,
                 mouse_dx, mouse_dy, mouse_buttons, goal_ids):
        import h5py
        with h5py.File(path, "w") as f:
            ...
            f.create_dataset("goal_ids", data=np.array(goal_ids, dtype=np.int8))  # ✅ 新增
            f.attrs["mode"] = mode
            f.attrs["fps"] = self.fps
            f.attrs["num_frames"] = len(frames)
            f.attrs["has_mouse"] = True
            f.attrs["num_goals"] = len(self._goals)  # ✅ 新增
            f.attrs["goals"] = ",".join(self._goals)   # ✅ 新增
    ...
    def _print_report(self, all_results, output_dir):
        print(f"\n{'=' * 60}")
        print(f"  录制质量报告 (Goal-Conditioned)")
        print(f"{'=' * 60}")
        ...
        for i, r in enumerate(all_results):
            ...
            # Goal 统计
            if "goal_counts" in r:
                for gid, cnt in r["goal_counts"].items():
                    total_goal_counts[gid] = total_goal_counts.get(gid, 0) + cnt

            print(f"\n  Episode {i+1}: {r['frames']} frames, {r['duration']:.1f}s, "
                  f"FPS={r['fps_actual']:.1f}")
            print(f"    动作分布:")
            ...
            # ✅ 新增：Goal 分布
            if "goal_counts" in r and r["goal_counts"]:
                print(f"    Goal 分布:")
                for gid, cnt in sorted(r["goal_counts"].items()):
                    goal_name = self._goals[gid] if gid < len(self._goals) else f"unknown({gid})"
                    print(f"      {goal_name}: {cnt} frames ({cnt / r['frames'] * 100:.1f}%)")
        ...
        # ✅ 新增：Goal 分布（汇总）
        if total_goal_counts:
            print(f"\n  Goal 分布（汇总）:")
            for gid in sorted(total_goal_counts.keys()):
                cnt = total_goal_counts[gid]
                goal_name = self._goals[gid] if gid < len(self._goals) else f"unknown({gid})"
                print(f"    {goal_name:20s} {cnt:5d} frames ({cnt / total_frames * 100:.1f}%)")
        ...
        # ✅ 新增：Goal 覆盖检查
        if self._goals:
            unreached = [i for i, r in enumerate(self._goal_reached) if not r]
            if unreached:
                issues.append(f"有 {len(unreached)} 个目标未到达（建议重新录制）")
            else:
                print(f"    ✅ 所有目标都已到达")
        ...
        print(f"\n  下一步:")
        print(f"    1. 预处理: C:\\Python\\python.exe training/preprocess_data_v2.py")
        print(f"    2. 训练:   C:\\Python\\python.exe training/goal_conditioned_bc.py")
        print(f"    3. 推理:   C:\\Python\\python.exe training/inference_goal.py --duration 60 --fps 10")
        print(f"{'=' * 60}")
    ...
    def collect(self, num_episodes=1, mode="pathfinding", skip_test=False,
        ...
        print(f"\n{'=' * 60}", flush=True)
        print(f"  WukongAI Data Collector v3.0 (Goal-Conditioned)", flush=True)
        ...
        if self._goals:
            print(f"  Goals: {len(self._goals)} goals", flush=True)
            for i, g in enumerate(self._goals):
                print(f"    {i}: {g}", flush=True)
            print(f"  ➡️  Press 'G' when you reach a goal!", flush=True)
        else:
            print(f"  ⚠️  No goals defined! Use --goals-file or --goals", flush=True)
        ...
        if self._goals:
            print(f"  ➡️  Current goal: {self._goals[self._current_goal_id]} (goal_id={self._current_goal_id})")
            print(f"  Press 'G' when you reach it!", flush=True)
        ...
        for ep in range(num_episodes):
            ...
            if self._goals:
                print(f"  ➡️  Current goal: {self._goals[self._current_goal_id]} "
                      f"(goal_id={self._current_goal_id}) | Press 'G' to advance", flush=True)
            ...
            while self._recording:
                ...
                goal_id = self.get_current_goal_id()  # ✅ 新增
            ...
            if len(ep_frames) > max(remaining_start, 10):
                ...
                self._save_h5(save_path, mode,
                              ...
                              ep_goal_ids[remaining_start:])  # ✅ 新增
            ...
            if len(ep_frames) > 10:
                ...
                result = {
                    ...
                    "goal_counts": goal_counts,  # ✅ 新增
                }
                all_results.append(result)
                print(f"\n  Episode {ep + 1} done: {result['frames']} frames, "
                      f"{result['duration']:.1f}s, FPS={result['fps_actual']:.1f}", flush=True)
                print(f"  Goal distribution:", flush=True)
                for gid in sorted(goal_counts.keys()):
                    gname = self._goals[gid] if gid < len(self._goals) else f"unknown({gid})"
                    print(f"    {gname}: {goal_counts[gid]} frames", flush=True)
            else:
    ...
    def _draw_hud_simple(self, frame, ep, total_ep, frame_count,
                         fps_actual, action_id, duration, goal_id):
        import cv2
        h, w = frame.shape[:2]
        overlay = frame.copy()
        ...
        cv2.putText(frame, f"REC Ep{ep}/{total_ep}  {frame_count}f  {fps_actual:.0f}fps  {duration:.0f}s",
        ...
        # ✅ 新增：显示当前 goal
        goal_name = self._goals[goal_id] if goal_id < len(self._goals) else "?"
        cv2.putText(frame, f"Goal: {goal_name} ({goal_id}) | Press 'G'",
                    (25, 95), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)

        keys_str = ",".join(sorted(self._keys_pressed)) if self._keys_pressed else "-"
        cv2.putText(frame, f"Keys: {keys_str}",
                    (25, 115), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (150, 150, 150), 1)
        return frame


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="WukongAI Data Collector v3.0 (Goal-Conditioned)")
    ...
    parser.add_argument("--goals-file", type=str, default=None, help="目标序列文件（每行一个目标）")
    parser.add_argument("--goals", type=str, nargs="+", default=None, help="目标序列（命令行指定）")
    parser.add_argument("--report", type=str, default=None, help="查看指定目录的录制报告")

    args = parser.parse_args()

    collector = GoalDataCollector(
    ...
    # 加载 goals
    if args.goals_file:
        collector.load_goals(args.goals_file)
    elif args.goals:
        collector.set_goals(args.goals)
    else:
        print("[Warning] No goals specified. Use --goals-file or --goals to define goals.", flush=True)
        print("Example: --goals-file goals.txt", flush=True)
        print("Example: --goals 存档点A 岔路口1 BOSS门", flush=True)
