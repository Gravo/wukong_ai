#!/usr/bin/env python3
"""Train a v5.6 baseline with historical frame stacking.

Differences from v5.5 clean baseline:
  ...
  - Uses historical frames [t-7, t-3, t-1, t] -> label[t].
"""
import argparse
...
class ManifestV56HistoryDataset(Dataset):
    """Preloaded v5.6 dataset using historical frame stacks."""
    ...
    def __init__(self, files: list[Path], verbose=True):
        ...
        for file_idx, h5_file in enumerate(tqdm(files, desc="preload", disable=not verbose)):
            with h5py.File(h5_file, "r") as hf:
                ...
                for frame_idx in range(LOOKBACK, n):
    ...
    def __getitem__(self, idx):
        ...
        mouse_dx = self.mouse_dx_cache[file_idx][frame_idx]
...
def train(args):
    ...
    train_ds = ManifestV56HistoryDataset(train_files, verbose=not args.quiet)
    ...
    write_split_manifest(output_dir / "v56_history_split_manifest.json", train_files, val_files, test_files, stats)
    status_path = output_dir / "status_v56_history.json"
    ...
    best_val = float("inf")
    best_action = -1.0
    log_path = output_dir / "training_log_v56_history.csv"
    ...
    try:
        for epoch in range(args.epochs):
            ...
            torch.save(checkpoint_payload, output_dir / "goal_bc_v56_history_last.pt")

            if val_metrics["loss"] < best_val:
                best_val = val_metrics["loss"]
                torch.save(
                    checkpoint_payload,
                    output_dir / "goal_bc_v56_history_best.pt",
                )
            if val_metrics["acc_action"] > best_action:
                best_action = val_metrics["acc_action"]
                torch.save(
                    checkpoint_payload,
                    output_dir / "goal_bc_v56_history_best_action.pt",
                )

        checkpoint = torch.load(output_dir / "goal_bc_v56_history_best.pt", map_location=device)
        model.load_state_dict(checkpoint["model_state_dict"])
        test_metrics = evaluate(model, test_loader, action_loss_fn, mouse_loss_fn, device)
        (output_dir / "test_metrics_v56_history.json").write_text(
...
def main():
    parser = argparse.ArgumentParser(description="Train v5.6 history-stack clean baseline")
    parser.add_argument("--manifest", required=True)
    parser.add_argument("--output-dir", default="checkpoints/v56_history_baseline")
