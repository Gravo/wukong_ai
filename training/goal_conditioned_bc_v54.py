"""
goal_conditioned_bc_v54.py - v5.4 训练脚本（7-bucket 鼠标分类）

改进（相比 v5.3）：
  1. 双头输出：Action Head (3-class) + Mouse Head (7-bucket 分类)
  2. 鼠标离散化：mouse_dx -> 7个bucket（快速左转~快速右转）
  3. 损失稳定：分类任务比回归更稳定

Bucket 定义：
  0: 快速左转   (mouse_dx <= -200)
  1: 中速左转   (-200 < mouse_dx <= -100)
  2: 慢速左转   (-100 < mouse_dx <= -20)
  3: 直行       (-20 < mouse_dx <= 20)
  4: 慢速右转   (20 < mouse_dx <= 100)
  5: 中速右转   (100 < mouse_dx <= 200)
  6: 快速右转   (mouse_dx > 200)
...
Loss：
  ...
  mouse_loss = CrossEntropyLoss(weights=[1.0, 2.0, 2.0, 1.0, 2.0, 2.0, 1.0])
  total_loss = action_loss + mouse_loss

使用方法：
  C:\Python\python.exe -u training\goal_conditioned_bc_v54.py ^
...
class FilteredH5Dataset(Dataset):
    ...
    双头标签：action class (3类) + mouse_bucket (7类)
    """
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            # 鼠标标签（7-bucket 分类）
            mouse_bucket = f["mouse_bucket"][frame_idx] if "mouse_bucket" in f else 3  # 默认直行
        ...
        return {
            ...
            "mouse_bucket": int(mouse_bucket),  # 0~6
            "goal_id": goal_id,  # 0/1
        }
...
class DualHeadModel(nn.Module):
    """
    双头模型：Action Head (3类) + Mouse Head (7类)
    """
    ...
    def __init__(self, num_actions=3, num_mouse_buckets=7, latent_dim=512, hidden_dim=256):
        ...
        # Mouse Head (7-class 分类)
        self.mouse_head = nn.Sequential(
            nn.Linear(self.feature_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(hidden_dim, num_mouse_buckets),  # 7个bucket
        )
    ...
    def forward(self, frames, goal_ids):
        ...
        mouse_logits = self.mouse_head(features)  # (B, 7)
...
def train_epoch(model, dataloader, optimizer, device, class_weights=None):
    ...
    # 鼠标损失（7-bucket 分类）
    # 权重：边缘bucket（0,6）样本少，降低权重
    mouse_weights = torch.tensor([0.5, 1.0, 2.0, 1.0, 2.0, 1.0, 0.5])
    mouse_criterion = nn.CrossEntropyLoss(weight=mouse_weights.to(device))
    ...
    for batch in pbar:
        ...
        mouse_buckets = batch["mouse_bucket"].to(device)  # (B,) 0~6
        ...
        mouse_loss = mouse_criterion(mouse_logits, mouse_buckets)
...
def eval_epoch(model, dataloader, device, class_weights=None):
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            goal_ids = batch["goal_id"].to(device)
            action_classes = batch["action_class"].to(device)
            mouse_buckets = batch["mouse_bucket"].to(device)
